...
describe('Testing native BigInt support: parse', function () {
  ...
  it('Should show JSONbig does support decimal and scientific notation parse/stringify roundtrip', function (done) {
    var JSONbig = require('../index')();
    var obj = JSONbig.parse(input);
    expect(obj.deci.toString(), 'decimal number').to.equal(
      '1234567890.0123456'
    );
    expect(typeof obj.deci, 'decimal number').to.equal('number');
    expect(obj.shortExp.toString(), 'short exponential number').to.equal(
      '1.79e+308'
    );
    expect(typeof obj.shortExp, 'short exponential number').to.equal('number');
    expect(obj.longExp.toString(), 'long exponential number').to.equal(
  ...
  it('Should infer correctly when to cast number to BigInt', (done) => {
    const data = {
      maxSafe: Number.MAX_SAFE_INTEGER,
      overSafe: Number.MAX_SAFE_INTEGER + 1,
    };
    const JSONbig = require('../index')();
    const stringified = JSONbig.stringify(data);
    const parsed = JSONbig.parse(stringified);
    expect(typeof parsed.maxSafe).to.equal('number');
    expect(typeof parsed.overSafe).to.equal('bigint');
    done();
  });
});
